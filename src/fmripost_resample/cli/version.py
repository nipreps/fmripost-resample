...
def check_latest():
    ...
    cachefile = Path.home() / '.cache' / 'fmripost_resample' / 'latest'
    try:
    ...
    if latest is None or outdated is True:
        ...
        with suppress(Exception):
            response = requests.get(
                url='https://pypi.org/pypi/fmripost_resample/json', timeout=1.0
            )
...
def is_flagged():
    ...
    # https://raw.githubusercontent.com/nipreps/fmripost_resample/main/.versions.json
    flagged = ()
    response = None
    with suppress(Exception):
        response = requests.get(
